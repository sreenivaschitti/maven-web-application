   ...
   stages{
            ...
            stage('deploy')
            {
                steps()
                {
                    sshagent(['docker_run']) 
                    {
                        sh 'ssh -o StrictHostKeyChecking=no ubuntu@54.226.218.27 docker rm -f mavenwebapplication || true'
                        sh 'ssh -o StrictHostKeyChecking=no ubuntu@54.226.218.27 docker run -d --name mavenwebapplication -p 8080:8080 sreenivaschitti/dockerpipeline:${buildNumber}'
                        }
                }
            }
